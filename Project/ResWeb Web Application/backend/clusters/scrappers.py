...
def getURLlist(root_url, strategies,depth):
    ...
    # Collected Urls in a given depth for a given url in the main url list
    all_urls2 = []
    ...
    for depthCount in range(depth):
        ...
        for murl in all_urls:
            ...
            try:
                ...
                all_urls_tags = content.find_all('a')
                ...
                print('Url not Crawled (Max tries Exceeded): ', murl['url'])
        ...
        # Passing all collected urls for the next depth
...
def parseAll(pdf_urls, word_urls, powerpoint_urls, text_urls, non_html_text_urls):
    ...
    # If URL List not empty the parse data
    if pdf_urls:
